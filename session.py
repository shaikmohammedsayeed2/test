...
class RleSession:
    ...
    def __init__(self, jwt_payload, no_session=False):
...
def get_rle_session(request: Request):
    ...
    if not rle_session_ck:
        return RleSession("No Cookie", no_session=True)
    try:
...
def get_person_role_by_email(email: str, lab_id: int, db: Session):
    ...
    lab_member = db.query(models.LabMember).filter(models.LabMember.person_id == person.id,
                                                   models.LabMember.lab_id == lab_id).first()
    if not lab_member:
        raise HTTPException(status_code=401, detail='User not authorized for this lab')
