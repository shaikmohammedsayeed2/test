...
async def get_publications(lab_id: int, user: RleSession = Depends(get_session), db: Session = Depends(get_db)):
    response = dict()
    upcoming_conference = db.execute(
        text(Path("sql/research_conference.sql").read_text().format(lab_id))).mappings().all()
...
async def update_conference(conf_id: int, conf: schemas.ConferenceUpdate, user: RleSession = Depends(get_session),
                            db: Session = Depends(get_db)):
    CHECK_ACCESS(user, USER_ROLE["user"])
...
async def update_publication(pub_id: int, pub: schemas.PublicationUpdate, user: RleSession = Depends(get_session),
                             db: Session = Depends(get_db)):
    CHECK_ACCESS(user, USER_ROLE["user"])
...
async def update_patent(patent_id: int, patent: schemas.PatentUpdate, user: RleSession = Depends(get_session),
                        db: Session = Depends(get_db)):
    CHECK_ACCESS(user, USER_ROLE["user"])
